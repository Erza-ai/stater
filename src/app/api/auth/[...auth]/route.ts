...
import { authServiceFetch, coreServiceFetch } from "@/lib/http/client";
import type {
  AuthSessionResponse,
  SessionLoadResult,
} from "../types";
...
async function proxyRequest(
  ...
  const res = await authServiceFetch(path, {
...
async function handleSessionLoad(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const sessionRes = await authServiceFetch("get-session", { token });
    if (!sessionRes.ok) {
  ...
  try {
    const productTokenRes = await authServiceFetch(`token/${env.PRODUCT}`, { token });
    ...
    const profileRes = await coreServiceFetch("/api/users", { token: bearerToken });
    if (profileRes.ok) {
